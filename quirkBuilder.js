...
const helpText = {
  ...
  replaceRegex: `<div><p>This is a string of text that will replace what is matched by the above regex. This tool is using the javascript regex format, thus matched groups can be inserted using "$1", "$2", "$3" and so on.</p><p>There are also 3 built in functions within both this tool and the pesterchum client, "upper()", which will uppercase any text within it's brackets, "lower()", which will lowercase anything within the brackets, and "scramble()", which will randomly reorder the characters within the brackets.</p></div>`,
